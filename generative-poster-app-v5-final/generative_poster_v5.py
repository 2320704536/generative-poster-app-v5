...
st.set_page_config(page_title="Generative Poster v5.1", layout="wide")
st.title("Generative Abstract Poster v5.1")
st.markdown("Interactive - Arts & Advanced Big Data  \nEnhanced Layout: Palette Preview placement & Streamlit compatibility fix")
...
def set_background(ax, mode):
    ...
    if mode == "Gradient":
        ...
        ax.imshow(np.dstack([grad,grad,grad]), extent=[0,1,0,1], origin="lower", zorder=-10)
...
def render_palette_preview(colors):
    ...
    for i, c in enumerate(colors):
        ax.add_patch(plt.Rectangle((i/n, 0), 1/n, 1, color=c, ec=(0,0,0,0)))
...
def draw_poster(shape="Blob", layers=8, wobble=0.15, palette_kind="Pastel", bg="Off-white",
    ...
    if seed not in (None, "", 0):
        try:
            ...
            pass
    if aspect=="Portrait": fig,ax=plt.subplots(figsize=(7,10))
    elif aspect=="Landscape": fig,ax=plt.subplots(figsize=(10,7))
    else: fig,ax=plt.subplots(figsize=(8,8))
    ax.axis("off")
    text_mode=set_background(ax,bg)
    cols=palette_override if palette_override else get_palette(palette_kind,7)
    for _ in range(layers):
        cx,cy=random.random(),random.random(); rr=random.uniform(r_min,r_max)
        color=random.choice(cols); alpha=random.uniform(alpha_min,alpha_max)
        if shape=="Blob": x,y=blob((cx,cy),rr,wobble=wobble); ax.fill(x,y,color=color,alpha=alpha)
        elif shape=="Polygon": x,y=polygon((cx,cy),sides=random.randint(3,8),r=rr,wobble=wobble); ax.fill(x,y,color=color,alpha=alpha)
        elif shape=="Star": x,y=star((cx,cy),points=random.randint(5,8),r1=rr,r2=rr*0.5); ax.fill(x,y,color=color,alpha=alpha)
        elif shape=="Spiral": x,y=spiral((cx,cy),turns=random.randint(2,4),r=rr); ax.plot(x,y,color=color,alpha=alpha,lw=2)
        elif shape=="Cloud": [ax.fill(x,y,color=color,alpha=alpha) for x,y in cloud((cx,cy),rr)]
    ...
    ax.set_xlim(0,1); ax.set_ylim(0,1); return fig

def fig_to_pil(fig):
    buf=io.BytesIO(); fig.savefig(buf,format="png",dpi=300,bbox_inches="tight"); buf.seek(0)
    return Image.open(buf).convert("RGBA"), buf
...
with st.sidebar:
    ...
    # Palette preview immediately below custom color
    ...
    st.header("Controls")
    shape = st.selectbox("Shape Type", ["Blob","Polygon","Star","Spiral","Cloud"])
    ...
    brightness = st.slider("Brightness",0.5,1.5,1.0,0.01)
    contrast = st.slider("Contrast",0.5,1.5,1.0,0.01)
    blur_amt = st.slider("Blur",0.0,5.0,0.0,0.1)
    seed_in = st.text_input("Seed (optional, int)", value="")
    if "reroll" not in st.session_state: st.session_state.reroll=0
    if st.button("Generate Random Poster"): st.session_state.reroll+=1

# ---------- Logic ----------
if seed_in.strip()=="":
    effective_seed = st.session_state.reroll if st.session_state.reroll>0 else None
else:
    try: base=int(seed_in)
    except: base=None
...
pil_img,raw_buf=fig_to_pil(fig)
if abs(brightness-1.0)>1e-6: pil_img=ImageEnhance.Brightness(pil_img).enhance(brightness)
if abs(contrast-1.0)>1e-6: pil_img=ImageEnhance.Contrast(pil_img).enhance(contrast)
if blur_amt>0: pil_img=pil_img.filter(ImageFilter.GaussianBlur(blur_amt))

st.image(pil_img, use_container_width=True)
out_buf=io.BytesIO(); pil_img.save(out_buf,format="PNG")
st.download_button("Download Poster as PNG",data=out_buf.getvalue(),file_name="poster_v5.1.png",mime="image/png")
