...
st.set_page_config(page_title="Generative Poster v5", layout="wide")
st.title("Generative Abstract Poster v5")
st.markdown("Interactive - Arts & Advanced Big Data  \nAdvanced Controls: Custom Color, Filters, Shapes, Aspect Ratio")
...
def spiral(center=(0.5,0.5), turns=3, points=500, r=0.4):
    ...
    x = center[0] + rad*np.cos(t); y = center[1] + rad*np.sin(t)
    return x, y
...
# ---------- Draw Poster (returns Matplotlib figure) ----------
...
with st.sidebar:
    ...
    st.caption("If enabled, the app will use a gradient generated from your picked color.")
    ...
    # Palette dropdown only shown when custom is OFF
    if not use_custom:
    ...
    preview_cols = custom_cols if use_custom else get_palette(palette_kind if palette_kind != "Custom Color" else "Pastel", 7)
...
st.download_button("Download Poster as PNG", data=out_buf.getvalue(), file_name="poster_v5.png", mime="image/png")

st.markdown("---")
st.caption(f"Palette: {('Custom ' + picked_hex) if use_custom else palette_kind} | Background: {bg_mode} | Shape: {shape} | Layers: {layers} | Wobble: {wobble:.2f} | Alpha: {alpha_min:.2f}-{alpha_max:.2f} | Size: {r_min:.2f}-{r_max:.2f} | Aspect: {aspect} | Brightness: {brightness:.2f} | Contrast: {contrast:.2f} | Blur: {blur_amt:.2f} | Reroll: {st.session_state.reroll}")
